...
class KernelBoundaryTests(unittest.TestCase):
    ...
    def test_vae_milles_without_lake(self) -> None:
        ...
        self.assertIsNone(outer.str_or_none(None))
        self.assertEqual(outer.str_or_none(1), "1")
        self.assertEqual(outer.as_dict({"a": 1}), {"a": 1})
        self.assertEqual(outer.as_dict("x", {}), {})
        self.assertIsNone(outer.as_dict("x"))
        self.assertEqual(outer.path_or("/tmp/x"), Path("/tmp/x"))
        ...
        self.assertEqual(outer.reason_text("skip"), "skip")
        self.assertEqual(outer.str_map({1: 2}), {"1": "2"})
        self.assertEqual(outer.str_keys({1: "a"}), {"1": "a"})
