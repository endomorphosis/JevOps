...
def merge_baselines(corpus: str, previous: dict, continuation: dict) -> dict:
    """Fill only unavailable pins with disjoint observations; never pick a winner.

    Both inputs retain the entire corpus matrix. Previously measured rows must
    be omitted from the continuation's project bindings, not run a second time.
    This combines saved claims, not live readiness or receipt authentication.
    """
    for report in (previous, continuation):
        build_report(corpus, report, [])
    if "continuation_sources" in continuation:
        raise ValueError("continuation must be a single native run")
    key = lambda row: (row["name"], row["lean_tag"], row["git_commit"])
    later = {key(row): row for row in continuation["rows"]}
    merged = copy.deepcopy(previous)
    inherited = previous.get("continuation_sources")
    previous_sha = source_hash(json.dumps(previous, sort_keys=True))
    next_sha = source_hash(json.dumps(continuation, sort_keys=True))
    origins = dict(inherited["row_sources"]) if inherited else {
        "|".join(key(row)): previous_sha for row in previous["rows"]}
    for i, old in enumerate(previous["rows"]):
        new = later[key(old)]
        if old["status"] != "UNAVAILABLE":
            if (new["status"] != "UNAVAILABLE" or "receipt" in new or "context" in new
                    or new.get("capability_gaps") != ["prepared_project_binding_missing_or_ambiguous"]):
                raise ValueError("continuation overlaps an earlier measured/attempted pin")
            continue
        if "receipt" in old or "context" in old:
            raise ValueError("cannot replace a previous attempted verification")
        if (new["status"] == "UNAVAILABLE" and "receipt" not in new and "context" not in new
                and new.get("capability_gaps") == ["prepared_project_binding_missing_or_ambiguous"]):
            continue  # An omitted binding adds no information about an old gap.
        # Even failed new checks replace old readiness gaps: never hide failure.
        merged["rows"][i] = copy.deepcopy(new)
        origins["|".join(key(old))] = next_sha
    merged["processes"] = previous["processes"] + continuation["processes"]
    merged["completed_version_checks"] = sum(
        r["status"] in {"VERIFIED", "REJECTED"} for r in merged["rows"])
    for field in ("fingerprint_file_reads", "fingerprint_stat_hits"):
        merged[field] = previous.get(field, 0) + continuation.get(field, 0)
    projects = {(p["repository"], p["lean_tag"], p["git_commit"]): p
                for p in previous.get("projects", [])}
    projects.update({(p["repository"], p["lean_tag"], p["git_commit"]): p
                     for p in continuation.get("projects", [])})
    merged["projects"] = list(projects.values())
    sources = inherited["reports"] if inherited else [previous_sha]
    if next_sha in sources:
        raise ValueError("duplicate continuation report")
    merged["continuation_sources"] = {"reports": [*sources, next_sha], "row_sources": origins,
        "semantics": "disjoint historical observations, not a fresh full-matrix run"}
    build_report(corpus, merged, [])
    return merged
...
def build_report(corpus: str, baseline: dict, trials: list[dict]) -> dict:
    ...
    result = {"schema": SCHEMA, "status": "COMPLETE" if all_verified else "INCOMPLETE",
    ...
    if "continuation_sources" in baseline:
        result["baseline_continuation"] = copy.deepcopy(baseline["continuation_sources"])
        result["notes"].append("Cumulative coverage from disjoint native runs; earlier checks were not rerun. "
                               "Per-row source hashes identify the originating reports.")
    return result


def summary(report: dict) -> str:
    processes_label = ("Reported native processes across contributing runs" if "baseline_continuation" in report
                       else "Reported fresh native processes")
    lines = ["# Full 15-problem benchmark", "", f"Status: **{report['status']}**.", "",
        ...
        f"{processes_label}: {report['reported_native_processes']}.", "",
...
def main() -> int:
    ...
    parser.add_argument("--continuation", action="append", default=[], type=Path,
                        help="full-matrix native report attempting only previously unavailable pins")
    parser.add_argument("--trial", action="append", default=[], type=Path)
    parser.add_argument("--output-dir", required=True, type=Path)
    args = parser.parse_args()
    if args.output_dir.exists():
        parser.error("output directory must be new")
    paths = [args.corpus, args.baseline, *args.continuation, *args.trial]
    ...
    for path in args.continuation:
        baseline = merge_baselines(corpus, baseline, json.loads(path.read_text()))
    report = build_report(corpus, baseline,
