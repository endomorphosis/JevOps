...
def tacticPP (module : ModuleName) (i: TacticInvocation) : IO String := do
  return (← moduleSource module).extract
    (i.info.stx.getPos?.getD 0)
    (i.info.stx.getTailPos?.getD 0)
...
def ppDeclWithoutProof (module: ModuleName) (info: CommandInfo) : IO String := do
    ...
    if info.stx[1][3][0].getAtomVal == ":=" then
      ...
      let decl := moduleSource.extract declStart proofStart
      let proof := moduleSource.extract proofStart proofEnd
      return decl
    else
      return ""
...
def trainingDataGivenTactic (elabDeclInfo : ElabDeclInfo) (module : ModuleName) (hash : String) (i : TacticInvocation) (declName : String) : IO TrainingData := do
  ...
  let sourceUpToTactic := (← moduleSource module).extract 0 (i.info.stx.getPos?.getD 0)
  let declUpToTactic := (← moduleSource module).extract
  ...
  let data := {
      declId := declId,
      declName := declName,
      srcUpToTactic := sourceUpToTactic,
      declUpToTactic := declUpToTactic,
      state := state,
...
def printTrainingDataGivenTheoremVal (elabDeclInfo : ElabDeclInfo) (module : ModuleName) (hash : String) (cmd : CompilationStep) (v : TheoremVal)
  ...
  Prod.fst <$> MetaM.toIO (ctxCore := coreContext) (sCore := coreState) do
    ...
    let sourceUpToTactic := (← moduleSource module).extract 0 (cmd.stx.getTailPos?.getD 0)
    let declUpToTactic := (← moduleSource module).extract (cmd.stx.getPos?.getD 0) (cmd.stx.getTailPos?.getD 0)
