...
class TaskQueue:
    def __init__(self, batch_size: int = 512, name: str = "test", log_interval: float = 60.0):
        self.name = name
            ...
            if time.time() - last_log_time >= self.log_interval:
                with self.lock:
                    if len(self._monitor_log) > 0:
                        print(
                            f"   TaskQueue-{self.name}:  {np.sum(self._monitor_log)} requests popped with avg batch_size {np.mean(self._monitor_log):.1f} in last period  {len(self.waiting_list)} waiting in queue",
                            flush=True,
                        )
                        self._monitor_log[:] = []
                last_log_time = time.time()
