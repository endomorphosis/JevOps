...
from lean_refactor.agents.state import APISearchResponseTypedDict
...
class TokenTracker:
    """
    Tracks token usage across LLM calls in a multi-agent pipeline.

    This class maintains a list of token usage records and provides
    methods for recording usage and generating summaries.
    """
    ...
    def get_summary_by_agent(self) -> dict[str, dict[str, int]]:
        """
        ...
        dict[str, dict[str, int]]
            Dictionary mapping agent names to their total input/output tokens.
        """
        summary: dict[str, dict[str, int]] = {}
        for record in self._records:
            if record.agent_name not in summary:
                summary[record.agent_name] = {"input_tokens": 0, "output_tokens": 0, "calls": 0}
            summary[record.agent_name]["input_tokens"] += record.input_tokens
            summary[record.agent_name]["output_tokens"] += record.output_tokens
            summary[record.agent_name]["calls"] += 1
        return summary
    ...
    def print_summary(self) -> None:
        ...
        # Per-call breakdown in order
        print("\nPer-Call Breakdown (in calling order):")
        print("-" * 70)
        print(f"{'#':<4} {'Agent':<30} {'Input':<12} {'Output':<12}")
        print("-" * 70)
        for record in self._records:
            print(
                f"{record.call_index:<4} {record.agent_name:<30} {record.input_tokens:<12} {record.output_tokens:<12}"
            )
        ...
        print("-" * 70)
        print(f"{'Agent':<30} {'Calls':<8} {'Input':<12} {'Output':<12}")
        print("-" * 70)
        agent_summary = self.get_summary_by_agent()
        for agent_name, stats in agent_summary.items():
            print(f"{agent_name:<30} {stats['calls']:<8} {stats['input_tokens']:<12} {stats['output_tokens']:<12}")

        # Totals
        print("\n" + "-" * 70)
        print(f"{'TOTAL':<30} {self.call_count:<8} {self.total_input_tokens:<12} {self.total_output_tokens:<12}")
        print(f"{'GRAND TOTAL TOKENS':<30} {self.total_tokens}")
        print("=" * 70 + "\n")
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        -------
        dict[str, Any]
            Dictionary containing all token usage data.
        """
        return {
            ...
            "per_agent_summary": self.get_summary_by_agent(),
            "calls": [
                {
...
def _extract_token_usage(response: BaseMessage) -> tuple[int, int]:
    ...
    Different providers store usage metadata in different places:
    - OpenAI: response.response_metadata['token_usage'] or response.usage_metadata
    ...
    # Try usage_metadata first (works for Gemini and newer LangChain versions)
    ...
    # Try response_metadata for OpenAI
...
def get_llm_response_content(
    ...
    """
    Extract the text content from an LLM response and optionally track token usage.

    Different LLM providers use different attributes to access response content:
    - OpenAI models use `.content`
    - Anthropic models use `.content`
    - Gemini models use `.text`

    Parameters
    ----------
    llm : BaseChatModel
        The LLM instance that generated the response.
    response : BaseMessage
        The response message from the LLM.
    token_tracker : TokenTracker | None
        Optional token tracker to record usage. If None, tokens are not tracked.
    agent_name : str
        Name of the agent making the call (used for tracking).
    ...
    if token_tracker is not None:
        ...
        token_tracker.record(agent_name, input_tokens, output_tokens)

    if isinstance(llm, ChatOpenAI) or isinstance(llm, ChatAnthropic):
        return str(response.content)
    # For Gemini and any other models, use .text
